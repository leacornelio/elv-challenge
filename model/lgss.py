...
class LGSSone(nn.Module):
    def __init__(self, cfg, mode="place"):
        ...
        if mode == "place":
            self.input_dim = (cfg['place_feat_dim']+cfg['sim_channel'])
            self.bnet = BNet(cfg)
        elif mode == "cast":
            self.bnet = BNet(cfg)
            self.input_dim = (cfg['cast_feat_dim']+cfg['sim_channel'])
        elif mode == "act":
            self.bnet = BNet(cfg)
            self.input_dim = (cfg['act_feat_dim']+cfg['sim_channel'])
        elif mode == "aud":
            self.bnet = BNet_aud(cfg)
            self.input_dim = cfg['aud_feat_dim']
        else:
            pass
